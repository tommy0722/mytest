...
function UpdatePage() {
  ...
  const addFoodItem = async () => {
    ...
    if (foodName) {
      ...
      const [data, error] = await performRequest('http://localhost:8000/api/foods/', 'POST', { body: newFoodItem });
      if (!error) {
  ...
  const handleSave = async (event) => {
    ...
    const [data, error] = await performRequest('http://localhost:8000/api/foods/batch_update/', 'PUT', { body: foodData });
  ...
  return (
    <div>
      ...
      <button className="col-1 btn btn-outline-success" type="button" onClick={handleSave}>保存</button>
        <div className="col-2"></div>
      <button className="col-1 btn btn-outline-primary" onClick={addFoodItem}>新增新食物</button>
        <div className="col-4"></div>
      </div>
    </div>
  );
